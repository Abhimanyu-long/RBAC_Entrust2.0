  ...
  const validate = async () => {
    ...
    if (credentials.dob.trim()) {
      ...
      if (dob == "Invalid Date") {
        ...
        if (!firstErrorField) firstErrorField = dobRef;
      }
      // Age validation (e.g., user must be at least 18)
    ...
    if (credentials.authorizedDateofBirth.trim()) {
      const dob = new Date(credentials.authorizedDateofBirth);
      const today = new Date();
      const age = today.getFullYear() - dob.getFullYear();
      const month = today.getMonth() - dob.getMonth();
      ...
      // Check if DOB is a valid date
      if (dob == "Invalid Date") {
        newErrors.authorizedDateofBirth = "Invalid date format.";
        if (!firstErrorField) firstErrorField = authorizedDobRef;
      }
      // Age validation (e.g., user must be at least 18)
      else if (age < 18 || (age === 18 && month < 0)) {
        newErrors.authorizedDateofBirth = "You must be at least 18 years old.";
        if (!firstErrorField) firstErrorField = authorizedDobRef;
      }
    }    
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        toast.error(`Invalid input. Please check your details and try again. ${response.response?.data.detail || ""}`);
