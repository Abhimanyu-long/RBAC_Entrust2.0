...
export const UserApprovals = () => {
  ...
  const handleRejectUser = async (approvalId) => {
    ...
    try {
      ...
      toast.success(`User rejected successfully: ${response.message}`);
      await fetchUsers();
    } catch (error) {
      console.error(error.message);
    }
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        console.log("Approval Response:", response.response);
        toast.error(
          response.response.data?.detail || "Error while approving the user."
        );
      }
    } catch (error) {
      const errorMessage =
        error.response.response?.data?.detail || error.message;
      toast.error(
        errorMessage || "An error occurred while approving the user profile."
      );
    }
  };
  ...
  const getStatusStyle = (user) => {
    if (user.is_approved === 1) return "badge-status-approved text-white";
    if (user.is_rejected === 1) return "badge-status-rejected text-white";
  ...
  return (
    <>
      ...
      >
        <div className="card mx-lg-4 mx-2 my-3 shadow-lg">
          <div
              ...
              <table className="table table-hover table-bordered align-middle mb-0">
                <thead className="bg-light-gray">
                  <tr>
                    ...
                    <th className="py-3 d-none d-sm-table-cell">Client Mail</th>
                ...
                <tbody className="bg-light">
                  {users.length > 0 ? (
                    users.map((user) => (
                      <tr key={user.user_id} className="position-relative">
                        ...
                        <td className="d-none d-sm-table-cell">
                          <div className="text-break">{user.client_email}</div>
                        </td>
                        <td>
                          <span className={`${getStatusStyle(user)}`}>
                            {getStatus(user)}
                          </span>
                        </td>
                        <td>
                          {user.is_approved === 0 && user.is_rejected === 0 ? (
                            <div className="d-flex flex-column flex-md-row flex-wrap gap-2">
                              <button
                                className="badge badge-light-success text-center fw-bold px-3 py-2 "
                                onClick={() =>
                                  handleUserApproval(
                                    user.approval_id,
                                    user.email_verified,
                              ...
                              {(user.email_verified === 0 ||
                                ...
                                <button
                                  className="badge badge-light-blue fw-bold px-3 py-2"
                                  onClick={() =>
